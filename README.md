# Ömer Faruk Ceylan
<p align="center">
  <img src="WhatsApp%20Image%202026-03-25%20at%2014.43.40.jpeg" width="180" height="180" style="border-radius: 50%; object-fit: cover; border: 3px solid #2c3e50;" alt="Ömer Faruk Ceylan">
  <br>
  <b>İnsansız Araç Teknolojileri Teknikeri</b>
</p>

## Hakkımda
Sakarya Uygulamalı Bilimler Üniversitesi, İnsansız Araç Teknolojileri bölümü öğrencisiyim. Lise eğitimimi Uçak Bakımı ve Uçak Elektroniği üzerine tamamladım. Otonom sistemler, endüstriyel otomasyon ve siber güvenlik alanlarında teknik çalışmalar yapmaktayım.

---

## Eğitim Bilgileri

| Kurum | Program | Tarih |
| :--- | :--- | :--- |
| **Sakarya Uygulamalı Bilimler Üniversitesi** | İnsansız Araç Teknolojileri | 2024 — Devam Ediyor |
| **Başiskele Selim Yürekten MTAL** | Uçak Bakımı / Uçak Elektroniği (ATP) | 2019 — 2023 |

**Akademik Not:** Lise Mezuniyet Puanı: 66,99

---

## İş ve Staj Deneyimi

* **3 Faz Otomasyon Elektrik Taahhüt** | *Teknik Stajyer*
    * Endüstriyel otomasyon sistemlerinin kurulumu ve elektrik pano montajı süreçlerinde saha çalışmaları.
* **Hakmar Mağazacılık** | *Operasyon ve Satış*
    * 4 ay süresince operasyonel süreçlerin yönetimi ve stok takibi.

---

## Teknik Yetkinlikler

| Kategori | Yetkinlikler |
| :--- | :--- |
| **Otonom & Donanım** | İHA Sistem Tasarımı, PID Kontrol, PLC, Sensör Entegrasyonu |
| **Yazılım & Programlama** | Python, C++, Arduino, HTML/CSS, Görüntü İşleme |
| **Siber Güvenlik** | Sızma Testleri, Metasploit, Burp Suite, Kali Linux |

---

## Projeler

### Ball and Beam (Top ve Kiriş) Dengeleme Sistemi
Arduino ve ultrasonik sensör mimarisi üzerinde, PID kontrol algoritması kullanılarak geliştirilen otonom dengeleme platformu.

---

## İlgi Alanları

* Havacılık ve Uzay Teknolojileri
* Gömülü Sistemler ve Yazılım
* Elektronik Devre Tasarımı
* Balık Tutma ve Amatör Balıkçılık

---

## İletişim Bilgileri

| Yöntem | Bilgi |
| :--- | :--- |
| **Telefon** | 0530 854 91 05 |
| **E-posta** | omerceylan073@gmail.com |
| **Konum** | İzmit, Kocaeli |
| **GitHub** | [github.com/deepsyntaxx](https://github.com/deepsyntaxx) |
